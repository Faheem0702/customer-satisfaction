...
<%
	...
	//
							if(paramname.equalsIgnoreCase("servicereq"))
							{
								service=multi.getParameter(paramname);
							}
//
						...
						//image = image.replace(".", "_b.");
			 			String query1="select * from apms_owner where name='"+uname+"'"; 
					...
					if ( rs1.next() )
					   ...
					   {
						   if(cloud.equalsIgnoreCase("1000"))
						   {
							   int val=1;
							   Calendar cal = Calendar.getInstance();
								...
								SimpleDateFormat sdfDate = new SimpleDateFormat("dd/MM/yyyy");
								 service = sdfDate.format(cal.getTime());
						   }
						   
						   if(cloud.equalsIgnoreCase("5000"))
						   {
							   int val=1;
							   Calendar cal = Calendar.getInstance();
						   ...
						   if(cloud.equalsIgnoreCase("10000"))
						   {
							   int val=2;
							   Calendar cal = Calendar.getInstance();
						...
						PreparedStatement ps=connection.prepareStatement("insert into apms_owner(name,pass,email,mobile,addr,dob,location,cloudname,servicetime,imagess) values(?,?,?,?,?,?,?,?,?,?)");
